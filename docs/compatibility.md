...
| Item | Impact |
|---|---|
| **CLI truststore default** | `provisionconfig` without `-e/--truststore` does not verify the management service's certificate — it only suppresses the warning. Pass the CA bundle explicitly on a managed fabric. Measured against ePO 5.10 (JMLLER-2025): the ePO web certificate is issued by the server's own `Orion_CA`, so the system trust store never verifies it; with the Orion CA as truststore the check passes only when ePO is addressed by its certificate name, and on **Python 3.13+** it still fails because the Orion CA carries no Key Usage extension and Python now verifies in `VERIFY_X509_STRICT` mode. A hardened default therefore needs an escape hatch for strict mode as well as `--insecure`. |
